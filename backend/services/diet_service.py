...
from backend.models.dimension import System, MealDeviation, SystemScoreLog, DEFAULT_SYSTEM_DETAILS
...
class DietService:
    ...
    def update_fuel_baseline(db: Session, request: FuelBaselineUpdate) -> Tuple[dict, int]:
        ...
        return DietService.get_fuel_baseline(db)
    ...
    def create_meal_deviation(db: Session, request: MealDeviationCreate) -> Tuple[dict, int]:
        ...
        # 更新系统统计
        DietService._update_fuel_statistics(db, system, deviation.id)
    ...
    def get_meal_deviations(
        ...
        score_info = ScoreInfo(
            current_score=system.score,
            consistency=details.get("consistency", 100),
            total_deviations=details.get("total_deviations", 0),
            monthly_deviations=details.get("monthly_deviations", 0)
        )
        ...
        paginated_data = {
            ...
            "has_prev": page > 1,
            "score_info": score_info.model_dump()
        }
    ...
    def delete_meal_deviation(db: Session, deviation_id: int) -> Tuple[dict, int]:
        ...
        # 更新系统统计
        DietService._update_fuel_statistics(db, system)
    ...
    def _log_score_change(
        ...
        db.add(log)
        db.flush()  # 确保日志写入但不提交事务

    @staticmethod
    def _update_fuel_statistics(db: Session, system: System, deviation_id: Optional[int] = None):
        """
        更新饮食系统统计（内部方法）
        ...
        # 记录评分变化日志
        DietService._log_score_change(
            db=db,
            system=system,
            old_score=old_score,
            new_score=consistency_score,
            change_reason="偏离事件" if deviation_id else "统计更新",
    ...
    def get_score_history(
        ...
        # 查询评分日志
        logs = db.query(SystemScoreLog).filter(
            SystemScoreLog.system_id == system.id,
            SystemScoreLog.created_at >= start_time
        ).order_by(SystemScoreLog.created_at.desc()).all()

        # 构建响应
        history = [SystemScoreLogResponse.model_validate(log) for log in logs]
